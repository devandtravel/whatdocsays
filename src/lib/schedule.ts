...
import { MedicationPlan, ScheduleEvent, Frequency } from '../types';
...
const makeDoseLabel = (plan: MedicationPlan, instructionIndex: number): string => {
...
const timesForInstruction = (frequency: Frequency, times?: string[]): string[] => {
...
export const expandPlan = (
  ...
  plan.instructions.forEach((instruction, instructionIndex) => {
    const times = timesForInstruction(instruction.frequency, instruction.timesOfDay);
    ...
    for (let dayOffset = 0; dayOffset < horizonDays; dayOffset += 1) {
      ...
      times.forEach((time) => {
        ...
        const occurrence = startOfDay.add(dayOffset, 'day').hour(hour).minute(minute).second(0);

        events.push({
          id: createEventId(plan.id, instructionIndex, dayOffset, time),
          medPlanId: plan.id,
          at: occurrence.toISOString(),
          windowMins: instruction.frequency === 'PRN' ? undefined : DEFAULT_WINDOW_MINS,
