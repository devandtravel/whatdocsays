...
type UseCameraPermissionState = ReturnType<
...
const useSafeCameraPermission =
	visionCameraModule?.useCameraPermission ?? useUnavailableCameraPermission;
const useSafeCameraDevice: UseCameraDevice =
	visionCameraModule?.useCameraDevice ??
	(() => undefined as ReturnType<UseCameraDevice>);
...
export default function CaptureScreen() {
	...
	const capturePhoto = useCallback(async () => {
		...
		if (!CameraComponent) {
			setErrorMessage(
				"Camera capture requires a development build. Use the image picker instead.",
			);
			return;
		}
	...
	const cameraReady =
		cameraSupported && !!device && hasPermission && !cameraTimeout;
	const awaitingPermission =
		cameraSupported && !hasPermission && !cameraTimeout;
	...
	if (isLoading) {
		return (
			...
			>
				...
				<Text style={{ color: "#fff", marginTop: theme.spacing.md }}>
					Loading...
				</Text>
			</SafeAreaView>
		);
			...
			>
				...
				{isProcessing ? (
					...
					<View style={{ flexDirection: "row", gap: theme.spacing.md }}>
						{cameraSupported ? (
							<CaptureButton label="Capture" onPress={capturePhoto} />
						) : null}
						<CaptureButton
							label="Pick image/PDF"
							onPress={pickFromLibrary}
							variant={
								isSimulator || cameraTimeout || !cameraSupported
									? "solid"
									: "outline"
							}
						/>
...
const CaptureButton = ({
	...
	<Pressable
		...
		style={{
			...
			borderColor: "#fff",
			backgroundColor:
				variant === "solid" ? theme.colors.accent : "transparent",
		}}
	>
