...
export default function TodayScreen() {
  ...
  // Debug logging
  console.log('[TodayScreen] Component rendered, events:', events.length, 'medPlans:', medPlans.length);

  const planLookup = useMemo(() => new Map(medPlans.map((plan) => [plan.id, plan.name])), [medPlans]);
  ...
  const handleTaken = useCallback(async (event: ScheduleEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[TodayScreen] Notification cancel failed:', notifyError);
      }
  ...
  const handleSkip = useCallback(async (event: ScheduleEvent) => {
  ...
  const handleSnooze = useCallback(async (event: ScheduleEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[TodayScreen] Notification operations failed:', notifyError);
      }
    } catch (error) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      <View style={{ paddingHorizontal: theme.spacing.lg, paddingTop: theme.spacing.lg }}>
        <Text style={[theme.typography.title, { color: theme.colors.text }]}>Today</Text>
        ...
        >
          ...
          <Text style={{ color: '#fff', fontWeight: '500' }}>Capture new plan</Text>
        </Pressable>
      </View>
      <ScrollView
        ...
        {grouped.length === 0 ? (
          ...
          >
            <Text style={{ color: theme.colors.textMuted }}>
              No reminders scheduled for today yet. Capture a prescription to get started.
            </Text>
          </View>
        ) : (
          grouped.map(([time, eventItems]) => (
            ...
            >
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={[theme.typography.subtitle, { color: theme.colors.text }]}>{time}</Text>
                <Text style={{ color: theme.colors.textMuted }}>{dayjs(eventItems[0].at).format('MMM D')}</Text>
              </View>
...
const ActionButton = ({ label, onPress, color, disabled }: ActionButtonProps) => (
  <Pressable
    accessibilityRole="button"
    onPress={onPress}
    disabled={disabled}
